  ...
  try{
    ...
    for(int m=0; m<Gibbs_number; ++m){
      ...
      for(int k=0; k<p; ++k){
        ...
        U_beta_1 = ((alpha+rho*I(k))*((n2-1)*betayh(k) - sum(Sigma2.col(k)% pleiotropy_gamma)-rho*sum(x2left.col(k)%latent_beta%I)- 
        ...
        U_gamma_1 =((((n2-1)*betayh(k) -rho*sum(Sigma2.col(k)% latent_beta % I)- alpha*sum(Sigma2.col(k)% latent_beta)- 
        ...
        if(randu_number_pleiotropy<= Probability_gamma_pleiotropy){
          ...
          //randn: Generate a scalar, vector, matrix or cube with the elements set to random values with normalGaussian distribution, parameterised by mean mu and standard deviation sd
          //http://arma.sourceforge.net/docs.html#randn
      ...
      if(indicator > 0){
        ...
        double sample_rho_mean = (as_scalar((I % latent_beta).t()* betayh*(n2-1)-(I % latent_beta).t()*Sigma2*pleiotropy_gamma - 
      ...
      double sample_alpha_mean = (as_scalar(latent_beta.t()* betayh*(n2-1)-latent_beta.t()*Sigma2*pleiotropy_gamma-
                                  latent_beta.t()*Sigma2*(latent_beta % I)*rho)/sigma2z)*sample_alpha_variance;
      ...
      //randg: Generate a scalar, vector, matrix or cube with the elements set to random values from a gamma distribution
      // http://arma.sourceforge.net/docs.html#randg
      ...
      post_sigma2z_scale = 2/(z_norm + as_scalar(latent_beta.t()* Sigma2 * latent_beta)*alpha*alpha + as_scalar(pleiotropy_gamma.t()* Sigma2 * pleiotropy_gamma)+ 
                            ...
                            2*alpha*as_scalar(pleiotropy_gamma.t()*Sigma2*latent_beta)- 2*rho*as_scalar((latent_beta % I).t()* betayh*(n2-1))- 
                            2*as_scalar(pleiotropy_gamma.t()* betayh*(n2-1)) - 2 * alpha * as_scalar(latent_beta.t()* betayh*(n2-1)));
